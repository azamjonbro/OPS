...
async function routeToTools(userMessage, apiKey, recentTurns = []) {
  ...
  const historyMessages = (recentTurns || [])
    ...
    // Generous per-turn cap — long enough that "buni telegramdan yuborib ber" after a full
    // Billz period report still has the real report text to forward, not a truncated stub.
    .map((t) => ({ role: t.role === 'assistant' ? 'assistant' : 'user', content: String(t.content).slice(0, 6000) }));
...
- Yozish/yaratish/yuborish vositalarini (notion_create_task, calendar_create_event, calendar_update_event, calendar_delete_event, mail_send_email, telegram_send_message, contact_send_message, scheduler_create_automation, billz_create_product) FAQAT foydalanuvchi biror narsani ANIQ yaratish/qo'shish/yuborish/o'zgartirish/o'chirishni so'raganda chaqir. Agar niyat aslida biror narsani qidirish, o'qish yoki topish bo'lsa (masalan "borib ... ni topib ... yubor"), avval qidiruv/o'qish vositasini ishlat, yozish vositasini chaqirma.
...
- TELEGRAM/EMAIL "KIM YOZDI": kanal aytilmasa ("kimlar yozdi", "yangilik bormi") telegram_read_messages VA mail_read_unread ikkalasini chaqir; faqat bitta kanal aytilsa ("telegramdan/emaildan kim yozdi") faqat o'shani chaqir.
- KIMGADIR YOZISH — KANAL TANLASH: "telegramdan/telegramda yoz" yoki tanish Telegram ismi/username bo'lsa ("azamjonbro ga yoz") -> telegram_send_message (person bilan, chatId shart emas). Aniq email yoki "emaildan yoz" bo'lsa -> mail_send_email. Kanal aytilmay "kimnidir top va yoz" deyilsa -> contact_send_message. Telegram uchun matnni rasmiy emas, qisqa va samimiy, egasi shaxsan yozgandek yoz.
- MUROJAAT QOIDASI (telegram_send_message / contact_send_message): xabar matnida odamning ismidan keyin, ismning jinsiga qarab "aka" (erkak) yoki "opa" (ayol) qo'sh — masalan "Bahodir aka, ...", "Nilufar opa, ...". Jinsni ismning oxirgi harfidan aniqla: ism "-a" bilan tugasa (Diyora, Feruza, Malika, Nilufar kabi -r bilan tugaydiganlar bundan mustasno — "Nilufar" ayol ismi lekin "a" bilan tugamaydi, taniqli ayol ismi sifatida bil) odatda ayol, undosh harf bilan tugasa (Diyor, Feruz, Sardor kabi) odatda erkak. AYNIQSA farqiga bor: "Diyor" (erkak, -> aka) va "Diyora" (ayol, -> opa) yoki "Feruz" (erkak, -> aka) va "Feruza" (ayol, -> opa) kabi bir-biriga o'xshash er/ayol juftliklarini ADASHTIRMA — ismning aniq oxirini harfma-harf tekshir.
- "BUNI/SHUNI/YUQORIDAGINI KIMGADIR YUBOR" QOIDASI (JUDA MUHIM): agar xabar "buni/shuni/yuqoridagi hisobotni/javobni ...ga yubor/jo'nat" desa, bu ma'lumotni QAYTA OLIB KELISH (billz/mail/boshqa o'qish vositasini qayta chaqirish) DEGANI EMAS — SUHBAT TARIXIDAGI oxirgi Assistant javobining o'zini olib, aynan o'sha mazmunni (kerak bo'lsa qisqa, o'qish uchun qulay qilib) yuborish vositasining matn/body argumentiga qo'yib, faqat o'sha yuborish vositasini (telegram_send_message/mail_send_email/contact_send_message) chaqir. Ma'lumot olish vositasini qayta chaqirma — u allaqachon tarixda bor.
