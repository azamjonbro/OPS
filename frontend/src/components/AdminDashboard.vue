  ...
  <div class="min-h-screen bg-[#0B0C0E] text-gray-100 font-sans flex flex-col">
    ...
    <div class="border-b border-white/5 bg-[#0E1013] px-6 py-2 flex items-center gap-2 text-xs font-medium text-gray-400">
      <button 
        ...
        :class="['px-4 py-2 rounded-xl transition', activeTab === 'integrations' ? 'bg-indigo-600/20 text-indigo-300 border border-indigo-500/30' : 'hover:bg-white/5 hover:text-white']"
      >
        🔌 Connections Hub ({{ integrations.length }})
      </button>
      <button 
        @click="activeTab = 'models'" 
        :class="['px-4 py-2 rounded-xl transition', activeTab === 'models' ? 'bg-indigo-600/20 text-indigo-300 border border-indigo-500/30' : 'hover:bg-white/5 hover:text-white']"
      >
        🧠 Dual AI Models (OpenAI + Claude)
      </button>
      <button 
        @click="activeTab = 'logs'" 
        :class="['px-4 py-2 rounded-xl transition', activeTab === 'logs' ? 'bg-indigo-600/20 text-indigo-300 border border-indigo-500/30' : 'hover:bg-white/5 hover:text-white']"
      >
...
export default {
  ...
  methods: {
    ...
    async fetchIntegrations() {
      try {
        const res = await axios.get('http://localhost:4000/api/admin/integrations');
        this.integrations = res.data;
      } catch (e) {}
    },
    async fetchModels() {
      try {
        const res = await axios.get('http://localhost:4000/api/admin/models');
        this.models = res.data;
      } catch (e) {}
    },
    async fetchLogs() {
      try {
        const res = await axios.get('http://localhost:4000/api/admin/logs');
        this.logs = res.data;
      } catch (e) {}
    },
    async fetchDualConfig() {
      try {
        const res = await axios.get('http://localhost:4000/api/admin/llm/dual-config');
        this.dualConfig = res.data;
      } catch (e) {}
    },
    async saveDualConfig() {
      try {
        await axios.post('http://localhost:4000/api/admin/llm/dual-config', this.dualConfig);
    ...
    async setDefaultModel(id) {
      try {
        await axios.post(`http://localhost:4000/api/admin/models/${id}/default`);
        this.fetchModels();
      } catch (e) {}
    },
    async testHealth(type) {
      try {
        const res = await axios.post(`http://localhost:4000/api/admin/integrations/${type}/test`);
    ...
    async handleSaveCredentials(payload) {
      try {
        await axios.post(`http://localhost:4000/api/admin/integrations/${payload.type}`, payload);
